...
MobileGallery = (function(_super) {
  ...
  MobileGallery.prototype._touchstart = function(e) {
    this.startPosX = event.touches[0].clientX;
  ...
  MobileGallery.prototype._touchmove = function(e) {
    e.preventDefault();
    return this.endPosX = event.touches[0].clientX;
  };
  ...
  MobileGallery.prototype._touchend = function(e) {
    ...
    if (this.isMulti) {
      ...
      detlaX = this.endPosX - this.startPosX;
      ...
      if ($image != null ? $image.length : void 0) {
        ...
        return this.image = $image.attr('src', src).show();
      }
    }
  };
  ...
  MobileGallery.prototype._getCurSrc = function($el) {
    ...
    src = $el.data('image-src') || $el.data('origin-src') || $img.attr('src');
    if ($.isFunction(this.opts.formatSrc)) {
      src = this.opts.formatSrc.call(this, src);
    }
    return src;
  };
