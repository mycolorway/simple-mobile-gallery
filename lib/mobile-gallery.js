...
MobileGallery = (function(_super) {
  ...
  MobileGallery.prototype._bind = function() {
    if (this.galleryImage.height() > this.winHeight) {
      $(document.body).on('touchstart.mobileGallery', this._touchstart);
    } else {
      this.gallery.on('touchmove.mobileGallery', function(e) {
        return e.preventDefault();
      });
    }
    return $(document.body).on('click', '.gallery-control #link-close', this.destroy);
  };
